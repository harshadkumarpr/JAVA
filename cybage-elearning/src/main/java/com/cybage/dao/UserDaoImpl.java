...
public class UserDaoImpl implements UserDao {
 ...
 public int udpateUser(User user) throws Exception {
	...
	public void addVideoHiistory(int videoid, String remoteUser) throws Exception {
		Connection con = DbUtil.getCon();
		String sql = "insert into videohistory (username,vid) values (?,?)";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setString(1, remoteUser);
		ps.setInt(2, videoid);
		ps.executeUpdate();
	}
	...
	public String getVideoStatus(int videoid, String remoteUser) throws Exception {
		Connection con = DbUtil.getCon();
		String sql = "select videostatus from videohistory where vid=? and username=?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, videoid);
		ps.setString(2, remoteUser);
	...
	public void updateVideoStatus(int videoid, String remoteUser) throws Exception {
		Connection con = DbUtil.getCon();
		String sql = "update videohistory set videostatus=? where username=? and vid=?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setString(1, "complete");
		ps.setString(2, remoteUser);
		ps.setInt(3, videoid);
	...
	@Override
	public String getCourseStatus(String remoteUser, int courseid) throws Exception {
		Connection con = DbUtil.getCon();
		String sql = "select status from enrollment where courseid=? and username=?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, courseid);
		ps.setString(2, remoteUser);
		ResultSet rs = ps.executeQuery();
