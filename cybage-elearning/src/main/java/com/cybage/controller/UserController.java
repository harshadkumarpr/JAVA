...
//      actual updation happens here...
		...
		if (path.equals("/confirm-enroll")) {
			...
			try {
				...
				for(Video v : video) {
					userService.addVideoHistory(v.getVideoid(),request.getRemoteUser());
				}
		...
		if (path.equals("/videoplay")) {
			...
			try {
				...
				String videostatus = userService.getVideoStatus(videoid,request.getRemoteUser());
				
				
				if(videostatus.equalsIgnoreCase("incomplete")) {
					
				int videohist = userService.getHistory(request.getRemoteUser(), courseid);
				System.out.println("video play history in enrollment  " + videohist);
				videohist++;
				userService.updateHistory(request.getRemoteUser(), courseid, videohist);
				userService.updateVideoStatus(videoid,request.getRemoteUser());
				}
				System.out.println("The video sequence "+videoseq);
			videoseq++;
				...
				String courseStatus = userService.getCourseStatus(request.getRemoteUser(), courseid);
				if (watch >= total && courseStatus.equalsIgnoreCase("incomplete")) {
					userService.updateStatus(request.getRemoteUser(), courseid);
					request.getRequestDispatcher("/coursecomplete.jsp").forward(request, response);
				} else {
